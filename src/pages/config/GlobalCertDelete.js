...
export default function GlobalCertDelete(props) {
  ...
  const cfgtype = props.location.state.data.cert.cfgtype;
  ...
  const url = '/config-server/certs/global?host=' + host + "&module=" + module + "&cfgtype=" + cfgtype + "&project=" + project + "&projver=" + projver + "&env=" + env + "&filename=" + filename;
  const headers = {};

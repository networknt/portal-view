...
export default function DeleteProperty(props) {
  ...
  const cfgtype = props.location.state.data.property.cfgtype;
  const scope = props.location.state.data.property.scope;
  const key = props.location.state.data.property.key;

  const url = '/config-server/configs/property?host=' + host + "&module=" + module + "&cfgtype=" + cfgtype + "&scope=" + scope + "&key=" + key;
  const headers = {};

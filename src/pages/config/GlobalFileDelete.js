...
export default function GlobalFileDelete(props) {
  ...
  const cfgtype = props.location.state.data.file.cfgtype;
  ...
  const url = '/config-server/files/global?host=' + host + "&module=" + module + "&cfgtype=" + cfgtype + "&project=" + project + "&projver=" + projver + "&env=" + env + "&filename=" + filename;
  const headers = {};

        ...
        <div className="container">
            <div {...getRootProps({style})}>
                ...
                {isDragActive ?
                    ...
                    <p>Drag 'n' drop the openapi.yaml here, or click to select the file</p>
                }
            </div>
        </div>
    )
    ...
    useEffect(() => {
        if (!mounted.current) {
            ...
            if(spec && !updated) {
                ...
                const domain = spec && spec.host ? spec.host.split('.').reverse().join('.') : '';
                let configDetail = { 
...
export default function PublishApi(props) {
    ...
    return (
        <div>
            <SpecDetail {...props} step={step} classes={classes} configDetail={configDetail} fileUpload={fileUpload}/>
            <ConfigDetail {...props} step={step} classes={classes} fileUpload={fileUpload} />
